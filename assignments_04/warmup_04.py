...
# 3. Find the threshold that maximizes the F1 score manually (using only existing imports)
best_f1 = -1
best_threshold = 0.5
best_tpr = 0
best_fpr = 0
...
# Note: Use 'classifier__C' to target the 'C' parameter of the 'classifier' step in the pipeline
param_grid = {
...
#
# The all-zeros sample will predict close to a 50/50 probability split (and usually Class 0 or 1 
# depending on the model's intercept term b).
#
# Reason:
# Standardizing features via StandardScaler converts raw values using z = (x - mean) / std.
# Since the synthetic features generated by make_classification are centered around zero,
# an all-zero input transforms into z-scores very close to zero.
# In Logistic Regression, the decision boundary equation is z = w1*x1 + w2*x2 + ... + b.
# When all inputs x are ~0, the weighted sum simplifies to just the intercept term (b).
# Because the dataset is balanced, the intercept b is close to 0, making logit(p) ≈ 0, 
# which yields a Sigmoid output probability p ≈ 0.50 (50%).
